...
const checkDogs = function (arr1, arr2) {
  //const arr = [...arr1];
  //const del1 = arr.shift();
  //const del2 = arr.pop();
  //const del2 = arr.splice(-2);
  const arrcopy = arr1.slice(1, -2);
  const arrconcate = [...arrcopy, ...arr2];
  console.log(arrconcate);
  arrconcate.forEach(function (val, i) {
    val >= 3
...
const calcAverageHumanAge1 = function (arr) {
  const dogAge = arr
    ...
    .filter(ele => ele > 18);
  //console.log(dogAge);
  let ava = dogAge.reduce((acc, cur, i, sarr) => {
    acc += cur;
    //console.log(sarr.length);
    if (i === sarr.length - 1) return acc / sarr.length;
    // console.log(i);
    return acc;
  });
  console.log(ava);
...
const calcAverageHumanAge2 = function (arr) {
  const dogAgeavarage = arr
    .map(ele => (ele <= 2 ? 2 * ele : 16 + ele * 4))
    .filter(ele => ele > 18)
    .reduce((acc, cur, i, sarr) => {
      acc += cur;
      if (i === sarr.length - 1) return acc / sarr.length;
      return acc;
    });
  console.log(dogAgeavarage);
};
...
dogs.forEach(function (el) {
  let recfood = Math.trunc(el.weight ** 0.75 * 28);
  el.recfood = recfood;
});
...
let rate = function (arr) {
  let rateEating = arr.find(function (el) {
    return el.owners.includes('Sarah');
  });
  rateEating.curFood > rateEating.recfood
...
//8. Create a shallow copy of the dogs array and sort it by recommended food portion in an ascending order (keep in mind that the portions are inside the array's objects)
let copyDogs = dogs.slice().sort((a, b) => a.recfood - b.recfood);
